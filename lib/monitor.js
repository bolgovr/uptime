...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  p = new Poller(check.url, this.config.timeout, function(err, time, result) {
    self.createPing(check, err, time, result, callback);
  });
  //p.setDebug(true);
  p.poll();
}

Monitor.prototype.createPing = function(check, error, time, result, callback) {
  status = error ? 'false' : 'true';
  var postData = 'checkId=' + check._id + '&status=' + status + '&time=' + time + '&name=' + this.config.name + '&result='+ result +'&error=' + (error ? error.message : '');
...
Monitor.prototype.updateAllChecks = function() {
  Stats.forEach(function (stat) {
    stat.updateLast24HoursQos.apply(stat);
    stat.updateLastHourQos.apply(stat);
  });
}
...
Monitor.prototype.aggregateQos = function() {
  ...
  var self = this;
  Stats.forEach(function (stat) {
    stat.cleanup(self.config.oldestHistory);
  });
